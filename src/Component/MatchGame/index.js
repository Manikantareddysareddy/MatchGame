...
class MatchGame extends Component {
  ...
  restartingGame = () => {
    this.setState({scoreCount: 0, timerCount: 60, showScoreCard: false})
    this.timerId = setInterval(this.descTime, 1000)
  }
  ...
  getFilteredImg = () => {
    ...
    const FilteredImgs = imagesList.filter(Item => Item.category === ActiveTab)
    return FilteredImgs
  }
  ...
  onMatchedImg = imageUrl => {
    const {randomImg, scoreCount} = this.state
    ...
    if (randomImg === imageUrl) {
      ...
      clearInterval(this.timerId)
      this.setState(prevState => ({timerCount: prevState.timerCount}))
      this.setState({
        showScoreCard: true,
        randomImg:
  ...
  render() {
    ...
    const FilteredImgs = this.getFilteredImg()
    ...
    return (
      <div className="bg-container">
        {showScoreCard ? (
          ...
          <div>
            <NavBar score={scoreCount} timer={timerCount} />

            <div className="card-container1">
              <div>
                <img src={randomImg} alt="match" className="matched-image" />
              </div>
              ...
              <ul className="ul-container1">
                {FilteredImgs.map(eachItem => (
                  <ThumbnailItem
                    thumbnailItem={eachItem}
                    key={eachItem.id}
                    onMatchedImg={this.onMatchedImg}
                  />
                ))}
              </ul>
            </div>
